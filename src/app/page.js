...
async function getProfiles(title, search) {
  const response = await fetch("https://web.ics.purdue.edu/~zong6/profile-app/fetch-data-with-filter.php?title=${title}&search=${search}&limit=1000",
    { next: { revalidate: 60 } }
  )
  const data = await response.json();
...
export default async function HomePage({searchParams}) {
  ...
  const [titles, profiles] = await Promise.all([getTitles(), getProfiles(title, search)]);
